...
（例：60）</span>',
	...
	/*グローバルメニュー*/
	$wp_customize->add_section( 'glonavi_section', array(
		'title' => __( 'グローバルメニュー設定', 'base' ),
		'priority' => 4,
    ));
...
//ここから下にスマホ専用のCSSを記述
	...
	$wp_customize->add_section( 'sns_section', array(
        ...
        'priority' => 12,
        'description' => 'FacebookやTwitterの設定',
    ));
...
function is_post_list_style(){
  return get_theme_mod( 'post_list_style', 'magazinestyle' );//第1引数にID、第2引数にデフォルト値を設定する
}
function is_sns_design_type(){
  echo get_theme_mod( 'sns_design_type', 'sns-design-type01' );//第1引数にID、第2引数にデフォルト値を設定する
}
function is_related_type(){
  return get_theme_mod( 'related_type', 'slide' );//第1引数にID、第2引数にデフォルト値を設定する
}
function is_related_title(){
  return get_theme_mod( 'related_title', 'RELATED POST' );//第1引数にID、第2引数にデフォルト値を設定する
}
function is_related_num(){
  return get_theme_mod( 'related_num', '3' );//第1引数にID、第2引数にデフォルト値を設定する
}
function is_toppage_style(){
  return get_theme_mod( 'toppage_style', 'two_column' );//第1引数にID、第2引数にデフォルト値を設定する
}
function is_pickup_style(){
  return get_theme_mod( 'pickup_style', 'menu-style' );//第1引数にID、第2引数にデフォルト値を設定する
}
function is_pickup_child(){
  return get_theme_mod( 'pickup_child', 'child_none' );//第1引数にID、第2引数にデフォルト値を設定する
}
function is_top_image_child(){
  return get_theme_mod( 'top_image_child', 'child_none' );//第1引数にID、第2引数にデフォルト値を設定する
}
function is_tw_name(){
  return get_option( 'tw_name', '' );//第1引数にID、第2引数にデフォルト値を設定する
}
function is_tw_type(){
  return get_option( 'tw_type', 'summary' );//第1引数にID、第2引数にデフォルト値を設定する
}
function cps_marker2_color(){
  return get_theme_mod( 'theme_marker2_type', '#a9eaf2' );//第1引数にID、第2引数にデフォルト値を設定する
}
