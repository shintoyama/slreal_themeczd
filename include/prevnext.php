			...
			<a class="prev" href="<?php echo get_the_permalink($prevpost->ID); ?>" title="<?php echo get_the_title($prevpost->ID); ?>">
				<div class="metabox">
					<?php if( has_post_thumbnail($prevpost->ID) ) :?>
						<?php echo get_the_post_thumbnail($prevpost->ID, 'cps_thumbnails'); ?>
					<?php else: ?>
						<img src="<?php echo get_template_directory_uri(); ?>/img/noimg320.png" alt="NO IMAGE" title="NO IMAGE" />
					<?php endif; ?>
			...
			<a class="next" href="<?php echo get_the_permalink($nextpost->ID); ?>" title="<?php echo get_the_title($nextpost->ID); ?>">
				<div class="metabox">
					...
					<?php if( has_post_thumbnail($nextpost->ID) ) :?>
						<?php echo get_the_post_thumbnail($nextpost->ID, 'cps_thumbnails'); ?>
					<?php else: ?>
						<img src="<?php echo get_template_directory_uri(); ?>/img/noimg320.png" alt="NO IMAGE" title="NO IMAGE" />
					<?php endif; ?>
				</div>
