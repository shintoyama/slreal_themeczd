...
function themeslug_enqueue_style() {
	...
	wp_enqueue_style( 'fontawesome-style', 'https://use.fontawesome.com/releases/v5.1.0/css/all.css' );
...
function themeslug_enqueue_script() {
	...
	wp_enqueue_script( 'fontowesome5', 'https://use.fontawesome.com/releases/v5.1.0/js/all.js', array( 'jquery' ), false, true );
}
...
function my_admin_print_styles() {
	...
	wp_enqueue_style( 'my-fa-script','https://use.fontawesome.com/releases/v5.1.0/css/all.css' );
}
...
function my_admin_print_scripts() {
	...
	wp_enqueue_script( 'my-fontowesome', 'https://use.fontawesome.com/releases/v5.1.0/js/all.js', array( 'jquery' ), false, true );
}
...
register_sidebar( array(
	'name' => 'サイドバー【PC】',
	'id' => 'sidebar',
	'description' => 'PCだけに表示されるウィジットエリアです。',
...
function description_in_nav_menu($item_output, $item){
    if( ! empty($item->description) ){
        $icon = '<span><i class="'.$item->description.'" aria-hidden="true"></i></span>';
        preg_match("|<a href=\"(.*?)\".*?>(.*?)|mis", $item_output, $matches);
        $url = $matches[1];
        $item_output = preg_replace('/<a .*?>(.*?)<\/a>/', "$1", $item_output);
        $item_output = '<a href="'.$url.'">'.$icon.$item_output.'</a>';
...
function deletecode($atts, $content = null) {
	$content = preg_replace('/\<p\>\<\/p\>', '', $content);
	return $content;
}
add_shortcode('code', 'deletecode');
...
function url_to_blog_card($the_content) {
	if ( is_singular() || is_category() || is_front_page() ) {
		...
		foreach ($m[0] as $match) {
			...
			$tag = '<a href="'.$url.'" class="blog-card"><div class="blog-card-hl-box"><i class="fas fa-file-alt"></i><span class="blog-card-hl"></span></div><div class="blog-card-box"><div class="blog-card-thumbnail">'.$thumbnail.'</div><div class="blog-card-content"><span class="blog-card-title">'.$title.'</span><span class="blog-card-excerpt">'.$excerpt.'...</span></div></div></a>';
